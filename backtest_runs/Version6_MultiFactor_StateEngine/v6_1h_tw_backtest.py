...
TEST_SYMBOLS = [
    ...
    {'code': '8114.TW', 'name': '振樺電'},
    {'code': '2455.TW', 'name': '全新'},
    {'code': '2451.TW', 'name': '創見'},
    {'code': '6274.TW', 'name': '台燿'},
]
...
def run_1h_simulation(df_dict, min_score=70):
    cash = INITIAL_CASH
    closed_trades = []
    ...
    for sym, df in df_dict.items():
        ...
        for i in range(20, len(df)):
            ...
            # 進場判斷 (得分 >= min_score)
            if not in_pos and sc >= min_score and st_dir == 1:
    ...
    if not closed_trades:
        return {'tot_ret': 0.0, 'win_rate': 0.0, 'pf': 0.0, 'mdd': 0.0, 'trades': 0}
    ...
    # MDD 計算
    eq_series = pd.Series([INITIAL_CASH] + [INITIAL_CASH + sum(t['pnl_twd'] for t in closed_trades[:i+1]) for i in range(len(closed_trades))])
...
def main():
    print("==========================================================================")
    print("🔬 台股 Version 6 策略 — 1H 小時線高精度量化回測 (近 60 天)")
    print("==========================================================================\n")

    df_dict_1h = {}
    print("📥 下載台股主力熱門標的高精度 1H 小時線 K 線數據...")
    for s in TEST_SYMBOLS:
        try:
            df = yf.download(s['code'], period='60d', interval='1h', progress=False)
            if isinstance(df.columns, pd.MultiIndex):
                df = df.xs(s['code'], axis=1, level=1)
            df = df.dropna(subset=['Close'])
            if len(df) > 50:
                df_dict_1h[s['name']] = calc_v6_1h_score(df)
        except Exception as e:
            pass

    print(f"✅ 成功載入 {len(df_dict_1h)} 檔台股主力股 1H 小時線數據！\n")

    scores = [60, 70, 75, 80]
    res_list = []
    for sc in scores:
        res = run_1h_simulation(df_dict_1h, min_score=sc)
        res_list.append({
            '1H 進場分數門檻': f"{sc} 分",
            '總報酬率 Net Ret (%)': f"{res['tot_ret']:+.2f}%",
            '勝率 Win Rate (%)': f"{res['win_rate']:.2f}%",
            '盈虧比 (Profit Factor)': f"{res['pf']:.2f}",
            '最大回撤 MDD (%)': f"{res['mdd']:.2f}%",
            '總交易筆數': res['trades']
        })

    print("🏆 【台股 1H 小時線多因子回測對比矩陣】")
    print("==========================================================================")
